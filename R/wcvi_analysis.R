...
#'
#' @examples
load_wcvi_data <- function(wcvi_catch_file, wcvi_effort_file) {
  ## Load in data ----
  data.sp <- read.csv(wcvi_catch_file, header = TRUE)
  data.ll <- read.csv(wcvi_effort_file, header = TRUE)
...
#'
#' @examples
clean_wcvi_urchin_trawls_data <- function(all.data) {
  ...
  urchin_trawls_IMPUTED <- 
    urchin_trawls_IMPUTED %>% 
    dplyr::filter(!is.na(Catch.count..pieces.) | !is.na(BEST_RETAINED_COUNT_rounded))
  ...
  return(urchin_trawls_IMPUTED)
}



clean_wcvi_all_trawls_data <- function(all.data) {
  trawl_depth <- all.data
...
#'
#' @examples
prepare_wcvi_density_data <- function(urchin_trawls_IMPUTED) {
...
prepare_wcvi_depth_data <- function(urchin_trawls_IMPUTED) {
...
prepare_wcvi_depth_bins_data <- function(urchin_trawls_IMPUTED) { 
...
#'
#' @examples
plot_fig8 <- function(wcvi_density, urchin_trawls_IMPUTED, wcvi_depth_bins) {
...
make_figure_9_data <- function(trawl_depth, wcvi_depth) {
  # Data setup
...
#'
#' @examples
plot_fig9 <- function(trawl_depth_dist) {
...
#'
#' @examples
plot_fig10 <- function(urchin_trawls_IMPUTED, wcvi_depth) {
  ...
  return(wcvi_depth_distribution_plot)
}


fit_wcvi_models <- function(trawl_depth_dist) {
...
plot_figS5 <- function(models) {
  ...
  quantile_plot <- function (model) {
    ggplot(model, aes(stats::qqnorm(.stdresid)[[1]], .stdresid)) + 
...
wcvi_kruskal_wallis_test <- function(urchin_trawls_IMPUTED) { 
  ...
  return(
    list(
      times = times,
      kw_times = kw_times,
      anova.data = anova.data
    )
  )
}


wcvi_ks_posthoc_test <- function(times, kw_times, anova.data) {
...
plot_figS1 <- function(all.data, urchin_trawls_IMPUTED) {
